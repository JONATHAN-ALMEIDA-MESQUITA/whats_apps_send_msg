...
def update_progress(current, total):
    ...
    global progress_label
    try:
        progress_text = f"Enviando {current}/{total} mensagens..."
        progress_label.config(text=progress_text, fg="blue")
        root.update()  # Força atualização da interface
    except:
        pass
...
def submitt():
    ...
    # Validar se há números para enviar
    if len(numbers) == 0:
        messagebox.showerror("No numbers", "Please provide at least one phone number")
        return
    ...
    if msg == "" and imagepath == "":
        ...
        if imagepath == "":
            print(f"Enviando para {len(numbers)} números")
            update_status(f"Iniciando envio para {len(numbers)} números...", "blue")
            ...
            for i in range(len(numbers)):
                if c == 1:
                    update_status("Abrindo WhatsApp Web...", "orange")
                    webbrowser.open("https://web.whatsapp.com")
                    time.sleep(8)  # Reduzido de 12 para 8
                    gui.keyDown('ctrl')
                    gui.press('w')
                    gui.keyUp('ctrl')
                    time.sleep(3)  # Reduzido de 8 para 3
                    c += 1
                
                update_progress(i+1, len(numbers))
                print(f"\n📤 Enviando mensagem {i+1}/{len(numbers)} para {numbers[i]}")
                
                url = f"https://web.whatsapp.com/send?phone={numbers[i]}&text={msg}&source&data&app_absent"
                webbrowser.open(url)
                time.sleep(6)  # Reduzido de 12 para 6
                gui.press('enter')
                ...
                successful_sends += 1
                update_status(f"✅ Mensagem {i+1} enviada para {numbers[i]}", "green")
            ...
            final_message = f"🎉 Processo concluído! {successful_sends}/{len(numbers)} mensagens enviadas com sucesso!"
            print(f"\n{final_message}")
            update_status(final_message, "green")
        else:
            ...
            # Versão otimizada e estável (com imagem)
            update_status(f"Iniciando envio com imagem para {len(numbers)} números...", "blue")
            c = 1
            successful_sends = 0
            for i in range(len(numbers)):
                if c == 1:
                ...
                successful_sends += 1
                update_status(f"✅ Mensagem {i+1} com imagem enviada para {numbers[i]}", "green")
            ...
            final_message = f"🎉 Processo concluído! {successful_sends}/{len(numbers)} mensagens com imagem enviadas com sucesso!"
            print(f"\n{final_message}")
            update_status(final_message, "green")
        imagepath = ""
        path = ""
...
# Labels para status e progresso (posicionados melhor)
progress_label = Label(root, text="", font="Helvetica 11", bg="snow", fg="blue")
progress_label.place(relx=0.02, rely=0.82, relwidth=0.96)

status_label = Label(root, text="Pronto para enviar mensagens", font="Helvetica 12 bold", bg="snow", fg="black")
status_label.place(relx=0.02, rely=0.85, relwidth=0.96)

submit = Button(root, text="Submit", font="impact 20 bold", fg="white", bg="green", command=submitt)
submit.place(relx=0.3, rely=0.90, relwidth=0.4)
